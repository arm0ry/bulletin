...
contract CollectiveTest_Sponsor is Test, CollectiveTest_Base {
    ...
    function testRevert_Sponsor_NotQualified() public {
        bytes memory payload;
        uint256 id = postProposal(
            owner,
            10,
        ...
        grantRole(address(bulletin), owner, bob, RELATIONS);
    ...
    function testRevert_Sponsor_Denied() public {
        bytes memory payload;
        uint256 id = postProposal(
            owner,
            10,
        ...
        vm.expectRevert(ICollective.Denied.selector);
        vm.prank(owner);
        collective.sponsor(id);
    }

    function testRevert_Sponsor_PropInProgress() public {
        bytes memory payload;
        uint256 id = postProposal(
            owner,
            10,
        ...
        vm.expectRevert(ICollective.PropInProgress.selector);
        vm.prank(bob);
        collective.sponsor(id);
    }
}
